...
<head>
  ...
  <style>
    .admin-input { width: 100%; padding: 10px; margin-bottom: 15px; background: #000; color: #fff; border: 1px solid rgba(0, 245, 255, 0.3); border-radius: 4px; }
    #edit-user-modal { display: none; position: fixed; inset: 0; background: rgba(10,10,15,0.9); z-index: 2000; align-items: center; justify-content: center; backdrop-filter: blur(5px); }
...
<body>
  ...
  <nav class="nav">
    <div class="nav-container">
      ...
      <ul class="nav-links">
        <li><a href="/store">Store Front</a></li>
      </ul>
      <div class="nav-auth">
              ...
              <?php if (!empty($users)): ?>
                  ...
                  <tr class="user-row" style="border-bottom: 1px solid #333;">
                    ...
                    <td style="padding: 10px;">
                      <span class="status-badge <?= $user['role']==='admin' ? 'active' : 'inactive' ?>" style="padding: 5px; border-radius: 4px; font-size: 0.8rem; border: 1px solid <?= $user['role']==='admin' ? 'var(--neon-green)' : 'gray' ?>;">
                    ...
                    <td class="user-actions" style="padding: 10px; display:flex; gap: 5px;">
                      <button class="btn btn-sm btn-primary" onclick="openEditUserModal(<?= $user['id'] ?>, '<?= htmlspecialchars(addslashes($user['username'])) ?>', '<?= htmlspecialchars(addslashes($user['email'])) ?>', '<?= htmlspecialchars(addslashes($user['role'] ?? 'client')) ?>')">Edit</button>
                      
                      <?php if ($user['id'] != $_SESSION['user_id']): ?>
                      <form method="POST" action="/admin/users/delete" style="margin:0;">
                          <input type="hidden" name="user_id" value="<?= $user['id'] ?>">
                          <button type="submit" class="btn btn-sm" style="background:#ff4444; color:white; border:none;">Ban</button>
                      </form>
                      <?php endif; ?>
                    </td>
                  </tr>
                  <?php endforeach; ?>
              <?php else: ?>
                  <tr><td colspan="6" style="padding: 20px;">No users found in database.</td></tr>
              <?php endif; ?>
            </tbody>
          </table>
        </div>
  ...
  <div id="edit-user-modal">
      <div class="modal-content">
          <button class="close-btn" onclick="document.getElementById('edit-user-modal').style.display='none'">&times;</button>
          <h2 style="color:var(--neon-cyan); margin-bottom: 20px; font-family: var(--font-display);">Modify User Protocol</h2>
          
          <form action="/admin/users/edit" method="POST">
              <input type="hidden" name="user_id" id="edit-user-id">
              
              <div>
                  <label class="setting-label" style="color:var(--text-secondary); display:block; margin-bottom:5px;">Username</label>
                  <input type="text" name="username" id="edit-user-username" class="admin-input" required>
              </div>
              
              <div>
                  <label class="setting-label" style="color:var(--text-secondary); display:block; margin-bottom:5px;">Email</label>
                  <input type="email" name="email" id="edit-user-email" class="admin-input" required>
              </div>
              
              <div>
                  <label class="setting-label" style="color:var(--text-secondary); display:block; margin-bottom:5px;">System Role</label>
